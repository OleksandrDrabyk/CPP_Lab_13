...
namespace lab_12
{
	class TSklad
	{
		...
		public DataGridViewComboBoxColumn cMeasureCB;
		public DataTable DovMeasure = new DataTable();
		...
		public TSklad()
		{
			...
			DataColumn cProvider = new DataColumn("Постачальник");
			DataColumn cMeasure = new DataColumn("Од.виміру");
			...
			DataColumn cTime = new DataColumn("Час");
			...
			cProvider.DataType = System.Type.GetType("System.String");
			cMeasure.DataType = System.Type.GetType("System.String");
			...
			TabSklad.Columns.Add(cProvider);
			TabSklad.Columns.Add(cMeasure);
			TabSklad.Columns.Add(cPrise);
			TabSklad.Columns.Add(cCount);
			TabSklad.Columns.Add(cVartist);
			TabSklad.Columns.Add(cTime);

			SkladView = new DataView(TabSklad); // Створюємо змінну типу DataView для можливості фільтрування і сортування
		}

		public void TSkladAddRow(string pNameGroup, string pNameProduct, string pProduser, string pProvider, string pMeasure, int pCount,
								 decimal pPrise, string pTime)
		{
			int nn;
		...
		public void ColumnPropSet(DataGridView DGV)
		{
			...
			DGV.Columns["Час"].HeaderText = "Час";
			...
			try // Пробуємо виконати
			{
				...
				using (StreamWriter sw = new StreamWriter(sNameFile))
				{
					foreach (DataRow rr in TabSklad.Rows)
					{
						...
						textRow = rr["Група"] + ";" + rr["Назва"] + ";" + rr["Виробник"] + ";" + rr["Постачальник"] + ";" + rr["Од.виміру"] + ";" +
						Convert.ToString(rr["Кількість"]) + ";" + Convert.ToString(rr["Ціна"]) + ";" + Convert.ToString(rr["Час"]);
		...
		{										  // Тут DGS – грід для виведення підсумків
			string sNameFile, textRow;
			string pGrupa, pNazva, pVyrobnyk, pPostachalnik, pOdVymiry, sKilkist, sCina, sTime; int pKilkist; decimal PCina; int i, ip;
			...
			using (StreamReader sr = new StreamReader(sNameFile))
			{
				...
				{
					pGrupa = ""; pNazva = ""; pVyrobnyk = ""; pOdVymiry = ""; pPostachalnik = ""; sKilkist = ""; sCina = ""; sTime = "";
					textRow = sr.ReadLine();
					...
					for (int j = ip; j <= i; j++)
					{
						pPostachalnik = pPostachalnik + textRow[j]; // Символи від ip до і-1 – виробник товару
					}
					ip = i + 2; // Запам’ятовуємо позицію третього розділювача
					i = textRow.IndexOf(';', ip) - 1; // Знаходимо наступний розділювач
					for (int j = ip; j <= i; j++)
					{
						pOdVymiry = pOdVymiry + textRow[j]; // Символи від ip до і-1 – виробник товару
					}
					ip = i + 2; // Запам’ятовуємо позицію третього розділювача
					i = textRow.IndexOf(';', ip) - 1; // Знаходимо наступний розділювач
					for (int j = ip; j <= i; j++)
					...
					{
						sTime = sTime + textRow[j]; // Символи від ip до і-1 – кількість товару
					}
					...
					TSkladAddRow(pGrupa, pNazva, pVyrobnyk,pPostachalnik, pOdVymiry, pKilkist, PCina,sTime);
			...
			catch (Exception e)
			{
				...
				MessageBox.Show("Немає джерела звідки можна було б зчитати таблицю");
			}
		}
		...
		public void CreateDovProvider()
		{
			// Оголошуємо public-змінну TabSkladSum типу DataTable
			// Таблиця DovGrupa буде складатись із одного стовпця – назва групи товару.
			DataColumn cNameProvider = new DataColumn("Постачальник");
			// Оголошуємо тип даних, які будуть зберігатись у стовпці
			cNameProvider.DataType = System.Type.GetType("System.String");
			DovProvider.Columns.Add(cNameProvider); // Додаємо стовпець до таблиці
			DataRow rowSklad0 = DovProvider.NewRow();
			rowSklad0[cNameProvider] = "ETTEX";
			DovProvider.Rows.Add(rowSklad0); // Додаємо сформований рядок до таблиці
			DataRow rowSklad1 = DovProvider.NewRow();
			rowSklad1[cNameProvider] = "Максус Плюс";
			DovProvider.Rows.Add(rowSklad1); // Додаємо сформований рядок до таблиці
			DataRow rowSklad2 = DovProvider.NewRow();
			rowSklad2[cNameProvider] = "Primaterra";
			DovProvider.Rows.Add(rowSklad2); // Додаємо сформований рядок до таблиці
			DataRow rowSklad3 = DovProvider.NewRow();
			rowSklad3[cNameProvider] = "ТІСА";
			DovProvider.Rows.Add(rowSklad3); // Додаємо сформований рядок до таблиці
			DataRow rowSklad4 = DovProvider.NewRow();
			rowSklad4[cNameProvider] = "SuShef";
			DovProvider.Rows.Add(rowSklad4); // Додаємо сформований рядок до таблиці
			DataRow rowSklad5 = DovProvider.NewRow();
			rowSklad5[cNameProvider] = "Biolands";
			DovProvider.Rows.Add(rowSklad5); // Додаємо сформований рядок до таблиці
			DataRow rowSklad6 = DovProvider.NewRow();
			rowSklad6[cNameProvider] = "Dropplatforma";
			DovProvider.Rows.Add(rowSklad6); // Додаємо сформований рядок до таблиці
			DataRow rowSklad7 = DovProvider.NewRow();
			rowSklad7[cNameProvider] = "Sat-Ellite";
			DovProvider.Rows.Add(rowSklad7); // Додаємо сформований рядок до таблиці
			DataRow rowSklad8 = DovProvider.NewRow();
			rowSklad8[cNameProvider] = "DekorHM";
			DovProvider.Rows.Add(rowSklad8); // Додаємо сформований рядок до таблиці
			DataRow rowSklad9 = DovProvider.NewRow();
			rowSklad9[cNameProvider] = "Gute";
			DovProvider.Rows.Add(rowSklad9); // Додаємо сформований рядок до таблиці
		...
		public void AddComboProvider(DataGridView DGV)
		{
			...
			// Джерело даних для нового стовпця – стовпець з іменем "Постачальник" у таблиці-джерелі даних.
			...
			DGV.Columns["Постачальник"].DisplayIndex = 4; // Встановлюємо порядок виведення стовпця у гріді
			DGV.Refresh(); // Оновлюємо DataGridView
		}
		...
		public void CreateDovMeasure()
		{
			// Оголошуємо public-змінну TabSkladSum типу DataTable
			// Таблиця DovGrupa буде складатись із одного стовпця – назва групи товару.
			DataColumn cNameMeasure = new DataColumn("Од.виміру");
			// Оголошуємо тип даних, які будуть зберігатись у стовпці
			cNameMeasure.DataType = System.Type.GetType("System.String");
			DovMeasure.Columns.Add(cNameMeasure); // Додаємо стовпець до таблиці
			DataRow rowSklad0 = DovMeasure.NewRow();
			rowSklad0[cNameMeasure] = "шт";
			DovMeasure.Rows.Add(rowSklad0); // Додаємо сформований рядок до таблиці
			DataRow rowSklad1 = DovMeasure.NewRow();
			rowSklad1[cNameMeasure] = "кг";
			DovMeasure.Rows.Add(rowSklad1); // Додаємо сформований рядок до таблиці
			DataRow rowSklad2 = DovMeasure.NewRow();
			rowSklad2[cNameMeasure] = "м";
			DovMeasure.Rows.Add(rowSklad2); // Додаємо сформований рядок до таблиці
			int nn = DovMeasure.Rows.Count;
		}

		public void AddComboMeasure(DataGridView DGV)
		{
			// Створимо екземпляр DataGridViewComboBoxColumn
			DataGridViewComboBoxColumn cMeasureCB = new DataGridViewComboBoxColumn();
			// Джерело даних для нового стовпця – стовпець з іменем "Постачальник" у таблиці-джерелі даних.
			cMeasureCB.DataPropertyName = "Од.виміру";
			cMeasureCB.Name = "cMeasureComb"; // Назва нового стовпця
			cMeasureCB.HeaderText = "Од.виміру"; // Заголовок на гріді нового стовпця
			cMeasureCB.DropDownWidth = 200; // Ширина "випадайки"
			cMeasureCB.Width = 120; // Ширина стовпця
			cMeasureCB.MaxDropDownItems = 7; // Кількість рядків випадайки, які одночасно будуть видимі
			cMeasureCB.FlatStyle = FlatStyle.Flat;
			cMeasureCB.ValueType = System.Type.GetType("System.string"); // Тип даних нового стовпця
			String s; Int32 n;
			n = DovMeasure.Rows.Count;
			// Тут DovGrupa – таблиця-довідник, яка містить допустимі значення назви групи товару
			foreach (DataRow r in DovMeasure.Rows) // Для кожного рядка r із таблиці DovGrupa
			{
				s = (string)r["Од.виміру"];
				cMeasureCB.Items.AddRange(r["Од.виміру"]); // Додаємо у "випадайку" елементи із довідника
			}
			DGV.Columns.Add(cMeasureCB); // Додаємо новий стовбець до гріда
										  // Перезаписати значення комірок із старого стовпця у новий
			String ss;
			// Для кожного рядка rr контрола DGV типу DataGridView
			foreach (DataGridViewRow rrr in DGV.Rows)
			...
			DGV.Columns["cMeasureComb"].Name = "Од.виміру"; // Перейменовуємо новий у старе ім’я
			DGV.Columns["Од.виміру"].DisplayIndex = 5; // Встановлюємо порядок виведення стовпця у гріді
			DGV.Refresh(); // Оновлюємо DataGridView
		}
	}
}
